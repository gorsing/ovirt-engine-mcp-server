...
def validate_name(value: str, field: str = "name") -> str:
    ...
    value = value.strip()
    if len(value) > 254:
        raise ValidationError(f"{field} must be at most 254 characters")
    return value
...
TOOL_VALIDATORS = {
    "vm_create": {
        "name": lambda v: validate_name(v, "VM name"),
        "cluster": lambda v: validate_name(v, "cluster"),
        "memory_mb": lambda v: validate_positive_int(
            v, "memory (MB)", min_val=256, max_val=1048576
        )
        if v
        else v,
        "cpu_cores": lambda v: validate_positive_int(
            v, "CPU cores", min_val=1, max_val=256
        )
        if v
        else v,
        "disk_size_gb": lambda v: validate_positive_int(
            v, "disk (GB)", min_val=1, max_val=65536
        )
        if v
        else v,
    },
    ...
    "vm_update_resources": {
        ...
        "memory_mb": lambda v: validate_positive_int(v, "memory (MB)", min_val=256)
        if v
        else v,
        "cpu_cores": lambda v: validate_positive_int(v, "CPU cores", min_val=1)
        if v
        else v,
    },
    "vm_rename": {
        "name_or_id": lambda v: validate_name_or_id(v),
        "new_name": lambda v: validate_name(v, "new name"),
    },
    "snapshot_create": {
        "name_or_id": lambda v: validate_name_or_id(v),
    ...
    "disk_create": {
        "name": lambda v: validate_name(v, "disk name"),
        "size_gb": lambda v: validate_positive_int(v, "disk (GB)", min_val=1),
    },
    "host_activate": {"name_or_id": lambda v: validate_name_or_id(v)},
    "host_deactivate": {"name_or_id": lambda v: validate_name_or_id(v)},
    "storage_attach": {
        "storage_name": lambda v: validate_name(v, "storage name"),
        "dc_name": lambda v: validate_name(v, "data center"),
    },
}
